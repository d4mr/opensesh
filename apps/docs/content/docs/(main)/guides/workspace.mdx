...
<Tabs items={["Event admins", "Reviewers"]}>
<Tab value="Event admins">
...
A collapsed sidebar: **Overview** and **My Reviews** — their [evaluation assignments](/guides/evaluation) and nothing else. Under blind review they don't see speaker identities either.

</Tab>
</Tabs>

<Callout title="Speakers and submitters are not members">
...
API keys live at the organization level, under **Organization settings → API keys**. A key (prefix `osk_`) authenticates as the organization for the [REST API](/api).

<Callout title="Treat a key like a password" type="warn">
