...
- **Dialect hygiene (future D1-or-Postgres deployments)**: the product will later support Postgres as an alternative deployment target, so SQLite-isms must never leak past the Db service. (1) Repos use the drizzle query builder only — no raw SQL fragments; a genuine escape hatch goes through one reviewed helper in the Db service. (2) The seed is TypeScript through drizzle inserts, not a .sql file. (3) Driver types, `batch`/transaction mechanics, and anything D1-specific stay inside the Db service; app and repo code never see them.
