...
- **Database: PlanetScale Postgres, reached from the Worker via Hyperdrive** (edge connection pooling; binding `HYPERDRIVE`, local dev via local connection string env). **Local dev loop runs against LOCAL Postgres, never prod**: Docker container `opensesh-pg` (postgres:18, matches prod 18.4) on port 5433; `apps/web/.dev.vars` `DATABASE_URL` points at it (`postgres://postgres:opensesh@localhost:5433/opensesh_dev`; the PlanetScale URL is the commented `#PROD_DATABASE_URL` line — swap only when explicitly told to touch prod data). `pnpm db:reset` / `pnpm seed` / dev server all read `.dev.vars`, so the whole loop is local and takes seconds. If the container is down: `docker start opensesh-pg`. **Drizzle owns schema and migrations**: `pg-core` table definitions, `drizzle-orm@rc` + `drizzle-kit@rc`, single flat init migration (see FLAT rule) applied with drizzle-kit. Real `jsonb`, `timestamptz`, pg enums where natural. Driver: postgres-js (or node-postgres if the Workers compat story demands) inside the Db service only. Try the official Effect integration `drizzle-orm/effect-postgres` ONCE against our pinned effect v4 (WP0 found `effect-core` broken vs beta.106 — it may be fixed in a newer rc); if it fails, keep the proven `Effect.tryPromise` wrapper pattern. No Durable Objects, no D1. Backend code runs in the Worker, in `createServerFn` handlers that run Effect programs through the shared runtime helper.
...
- **Monorepo (minimal)**: pnpm workspace + Vite+ (`vp run` with caching). Three packages — `apps/web` (the TanStack Start app: routes, components, server fns), `packages/domain` (drizzle tables, effect/Schema models, Effect services/repos, ICS — **no React, no TanStack imports; enforced by its package.json deps**), and `packages/email` (react-email templates; `pnpm --filter @opensesh/email dev` previews them at localhost:3010; exports sync render functions consumed by domain). `apps/web` imports `@opensesh/domain`; `@opensesh/domain` imports `@opensesh/email` for outbound mail. Do not add more packages without a spec saying so — in particular there is deliberately **no `apps/api`**: the public REST API is server routes in `apps/web/src/routes/api/v1/*` (thin handlers over domain services). It is ALSO exposed as **api.opensesh.io** — a wrangler route (`api.opensesh.io/*`) on the same Worker plus a host-based rewrite (`api.opensesh.io/v1/x` ≡ `/api/v1/x`; the api host serves a JSON index at `/`). Same deployable, vanity hostname. Root scripts fan out via `vp run` (`pnpm check` = all packages).
...
- **Read `docs/DESIGN.md` before building or restyling any surface.** It is the distilled taste reference — layout anatomy, recurring patterns (footer rails, progress rails, sync whispers, list rows, sliding indicators), applied motion recipes, and the do-not list. The rules below are the hard constraints; DESIGN.md is how they combine.
...
- **Flat UI, no harsh shadows — user directive.** "I prefer a flatter UI with outlines and surface differences that look nice. vercel style. over shadows and 3d." The entire Tailwind shadow scale is redefined soft+diffuse in `styles.css` `@theme` — registry components flatten automatically; do NOT edit their shadow classes and do NOT add shadow classes to app components. Depth = borders + surface tint contrast. One sanctioned exception: the floating demo-roles button keeps `shadow-lg` (it must stand out).
