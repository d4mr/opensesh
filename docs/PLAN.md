...
| Effect × Cloudflare | `@effect/sql-sqlite-do` exists in v4 — official SQLite client for Durable Objects. No D1 client |
...
- **The backend IS a Durable Object.** Verified: `@effect/sql-sqlite-do` adapts the DO's own `SqlStorage` handle, so Effect SQL executes inside the DO — we embrace it: one `Backend` DO class owns the ManagedRuntime (SqliteDO + Mail + R2 + Config layers from env) and exposes domain operations as **Workers RPC methods**. Server functions in Start are thin, typed forwarders: `createServerFn` → auth check → `env.BACKEND.op(...)`. Data is colocated with logic; every query is sub-ms; zero N+1 ceremony.
...
| WP | Scope | Depends on |
|---|---|---|
| **0. Scaffold** | TanStack Start on CF Workers, Tailwind v4 + greenroom.css wired, shadcn init, Effect v4 beta deps, **the critical spike**: `Backend` DO with sqlite-do + one migration + one RPC method → server fn → TanStack Query render, deployed to workers.dev. AGENTS.md, deploy script. *Decision gate: sqlite-do beta health (fallback: D1 + thin Effect wrapper, same DO-RPC shape).* | — |
