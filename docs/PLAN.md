...
- **Multi-tenancy**: row-level, minimal — `organizations` → `events` → everything else (already event-scoped). better-auth's `organization` plugin provides membership/roles logic over tables we own. One seeded org; no tenant UI for the competition — the data model + enforcement seam make SaaS-ification additive later. **Scale path (decided after evaluating options): Postgres** (schema- or DB-per-tenant — runtime-dynamic via connection strings, no deploy coupling), enabled by the dialect-hygiene rules. D1-per-tenant was evaluated and rejected as a default: dynamic tenant DBs require either the D1 HTTP API on the hot path (management-plane latency + account-wide token in the request path — unacceptable) or Workers for Platforms machinery (only worth it in a deliberate CF-platform play).
