...
| OS-003 | confirmed | codex WP28 | participantForEmail hardcodes role:"speaker"; cfp.ts matches by exact string; DevFlow seed defines "Primary speaker" → count 0. Fix: thread enabled roles into wizard payload + defensive coercion server-side. |
...
| OS-051 | confirmed gap | fix (cheap) | Overview lacks company/tag slices; raw data already client-side (workspace.directory). Add top-companies + tag panels with drill-through to filtered directory (same navigate path as Segments). |
