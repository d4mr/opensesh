...
1. **One `submissions` table for abstracts AND sessions** (`kind` column). Sessionboard shares a ~39-field catalog between them; the dashboard counts "accepted abstracts" and "accepted sessions" separately, so `kind` does not change on acceptance. The **agenda pool = accepted submissions of either kind**; manual/sponsor sessions are just `kind='session'` rows created already-accepted.
