...
- **Submenus (when/if a nav item needs children): follow `sidebar-03`** (user-designated: ui.shadcn.com/view/new-york-v4/sidebar-03) — its `SidebarMenuSub` composition is the only sanctioned submenu pattern. Current nav is flat; do not add submenus speculatively.
- **Personality stays — blocks are structure, greenroom is skin.** Adopting the blocks is NOT permission to ship shadcn's default neutral look: every surface renders through our greenroom tokens (stage green primary, warm paper neutrals, status palette), both modes. If a freshly installed block/component hardcodes zinc/slate classes, rebind them to tokens and note it.
- **Asset slots**: the user generates high-quality brand assets. Wire these as drop-in files under `apps/web/public/brand/` with graceful fallbacks (current text/tint treatment) until files exist: `logo.svg` + `logo-dark.svg` (sidebar OS-avatar + login heading + email header), `login-panel.jpg` (login-04 side panel, ~1600×2000 portrait), `og.png` (1200×630 social card). Reference by path; ship fallbacks so nothing breaks while slots are empty.
