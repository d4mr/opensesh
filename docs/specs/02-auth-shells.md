...
- Storage — **we own the tables**: better-auth's **Drizzle adapter** over the same D1 database, with its **user model mapped onto our existing `packages/domain` `users` table** (better-auth supports model/field mapping + `additionalFields`; extend `users` with whatever columns it requires — name/emailVerified/image etc. — via a normal drizzle migration). For `session`/`account`/`verification`: run `npx @better-auth/cli generate` ONCE as a reference, then write those tables as first-class definitions in `packages/domain/src/db/` (our naming conventions, snake_case), map better-auth to them, and migrate through our normal drizzle-kit flow. No auth-schema codegen artifacts checked in; better-auth supplies logic (sessions, magic links), our schema supplies 100% of storage.
...
- Identity model: better-auth `user` IS our `users` row (same table — no mapping indirection). Domain roles come from `event_members` (admin/reviewer); speakers link via `contacts` matched by email (contacts stay separate — they're event-scoped participant records, not identities). `CurrentUser` Effect service: session → `{ userId, email, roles: { admin: boolean, reviewer: boolean, contactId?: string } }`, typed `Unauthenticated`/`Forbidden` errors. Server fns declare required role via the runtime helper (`run(effect, { require: 'admin' })` or similar — keep it one option, not a framework).
